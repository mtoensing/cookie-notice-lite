...
function add_cookie_notice()
{
    ...
    $link = '';
    if ($url !== '') {
        $link = '<a style="font-size: small; text-align: right" rel="nofollow" href="' . esc_url($url) . '">' . esc_html__('More information', 'cookie-notice-lite') . '</a>';
    }
    ...
    echo '<p id="cookie-notice">' . esc_html($notice_text) . ' <br><button onclick="acceptCookie();"> ' . esc_html__('Okay', 'cookie-notice-lite') . '</button> ' . $link. ' </p>';
...
<p class="description"><?php echo esc_html__('Enter the text you would like to display for the cookie notice', 'cookie-notice-lite'); ?></p>
<?php
}
?>
